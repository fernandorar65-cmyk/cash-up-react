  ...
  return (
    <div className="mx-auto w-full max-w-7xl space-y-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          <nav className="mb-2 flex gap-1 text-xs font-semibold uppercase tracking-wider text-slate-500">
            ...
            <span>/</span>
            <span className="text-primary">Solicitudes de Crédito</span>
          </nav>
          <h1 className="text-3xl font-black tracking-tight text-slate-900 dark:text-slate-100">
            Aprobación y Desembolso
          </h1>
          <p className="text-slate-500 dark:text-slate-400">Expediente ID: #CSH-99283-B</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="flex items-center gap-4">
            <div className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border-2 border-primary/10 bg-slate-100 dark:bg-slate-800" />
            <div>
              <h3 className="text-lg font-bold text-primary dark:text-slate-100">Juan Carlos Pérez</h3>
        ...
        <div className="flex flex-col gap-4 rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
          <div className="mb-2 flex items-center gap-2">
            ...
            <h3 className="text-lg font-bold text-primary dark:text-slate-100">Propuesta de Crédito</h3>
          </div>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4 dark:border-slate-800">
            <div className="flex items-center justify-between text-sm">
              ...
              <span className="font-bold text-primary">$470.73</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="rounded-xl border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <h3 className="mb-6 flex items-center gap-2 text-xl font-bold text-primary dark:text-slate-100">
        ...
        <div className="flex flex-col gap-6">
          <div className="space-y-2">
            ...
            <textarea
              className="min-h-[100px] w-full rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 dark:border-slate-800 dark:bg-slate-800/50"
              placeholder="Ingrese comentarios internos..."
            />
          </div>
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap gap-3">
              <button className="flex items-center gap-2 rounded-lg bg-emerald-600 px-6 py-2.5 font-bold text-white transition-all hover:bg-emerald-700">
                <Icon name="check_circle" className="text-lg" /> Aprobar Crédito
              </button>
              <button className="flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-6 py-2.5 font-bold text-slate-700 transition-all hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-200">
                <Icon name="visibility" className="text-lg" /> Observar
              </button>
              <button className="flex items-center gap-2 rounded-lg border border-red-100 bg-red-50 px-6 py-2.5 font-bold text-red-600 transition-all hover:bg-red-100">
                <Icon name="cancel" className="text-lg" /> Rechazar
              </button>
            </div>
            <div className="hidden h-10 w-px bg-slate-200 lg:block" />
            <button
              className="flex cursor-not-allowed items-center gap-2 rounded-lg bg-primary/20 px-8 py-2.5 font-bold text-white/50"
              disabled
            >
      ...
      <section className="overflow-hidden rounded-xl border border-slate-200 bg-white opacity-80 shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <div className="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 p-6 dark:border-slate-800 dark:bg-slate-800/30">
            ...
            <thead className="bg-slate-50 text-[10px] font-bold uppercase tracking-wider text-slate-500 dark:bg-slate-800/50">
              <tr>
                {['N° Cuota', 'Fecha de Vencimiento', 'Capital', 'Interés', 'Seguro / Comis.', 'Total Cuota'].map((h) => (
                  <th
                    key={h}
                    className={`px-6 py-4 ${h === 'Total Cuota' ? 'font-bold text-primary' : ''}`}
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
              {[
                ...
                <tr key={r[0]}>
                  ...
                  <td className="px-6 py-4 font-bold text-primary">{r[5]}</td>
                </tr>
              ))}
